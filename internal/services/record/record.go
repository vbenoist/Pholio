...
func GetRecords(pgParams apimodels.PaginationQuery) (apimodels.PaginatedResults[databasemodels.Record], error) {
	...
	paginatedData, err := mongopagination.New(collection).Context(context.Background()).Filter(bson.D{}).Limit(pgParams.PerPage).Page(pgParams.Page).Decode(&documents).Find()
	if err != nil {
		return results, err
	}
