...
/* Based on https://github.com/gobeam/mongo-go-pagination/blob/master/pagingQuery.go */
...
type PagingQuery interface {
	...
	SortGroup(sortAsc int8) PagingQuery
...
// Sort group by results
func (paging *pagingQuery) SortGroup(sortAsc int8) PagingQuery {
	paging.SortGroupAsc = sortAsc
	return paging
}
...
func (paging *pagingQuery) Aggregate(filters ...interface{}) (paginatedData *PaginatedData, err error) {
	...
	ckg, err := checkIsGroupAgg(filters[0])
	if err != nil {
		return nil, err
	}

	if ckg && len(filters) == 1 {
		...
		/* Applying groupBy */
		facetData = append(facetData, filters[0].(bson.M))

		/* Sorting the groupped output */
		if paging.SortGroupAsc == 0 {
			paging.SortGroupAsc = -1
		}
		facetData = append(facetData, bson.M{"$sort": bson.M{"_id": paging.SortGroupAsc}})
	...
	debugJson, err := json.Marshal(aggregationFilter)
	if err != nil {
		return nil, err
	}
	fmt.Printf("AggregationFilter: %s\n\n", debugJson)
...
func checkIsGroupAgg(query interface{}) (isGroup bool, err error) {
